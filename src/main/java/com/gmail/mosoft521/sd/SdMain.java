package com.gmail.mosoft521.sd;

/**
 * Created by Alvin on 2016/12/12.
 */
public class SdMain {
    public static int[][] Initial_State = new int[10][10];

    public static void main(String[] args) {
        start();
        for (int i = 1; i <= 9; ++i) {
            for (int j = 1; j <= 9; ++j)
                System.out.println(Initial_State[i][j] + " ");
            System.out.println();
        }
    }


    public static boolean get_Initial_State(int i, int j)  //搜索第（ i , j ）位置处可以存储的数字,找到解则返回true，否则返回false
    {
        if (i > 9 || j > 9)
            return true;

        for (int k = 1; k <= 9; ++k) {
            boolean can = true;                // can 变量用于记录数字k能否放在 ( i , j ) 处
            for (int m = 1; m < i; ++m)
    ...
    public static void start() {
        //todo: srand ( unsigned ( time (NULL) ) );  /* 产生random_shuffle的随机数种子 */
        for (int i = 1; i <= 9; ++i)
            for (int j = 1; j <= 9; ++j)
                Initial_State[i][j] = 0;
        ...
        //todo:random_shuffle( &( Initial_State[1][1]) , &( Initial_State[1][10])  ) ;  /* 第一行随机排列产生 */
